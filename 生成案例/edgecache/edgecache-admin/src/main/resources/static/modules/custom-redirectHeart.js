/**
 @Name：RedirectHeart JS
 */
layui.define(['table', 'form'], function(exports) {
	var $ = layui.$,
		table = layui.table,
		form = layui.form,
		moduleUrl = "/redirect_heart/";
	table.render({
		elem: '#list-table',
		url: moduleUrl + 'page',
		method: 'post',
		...
		cols: [[
            ...
            {field: 'url', title: '检测地址',minWidth: 400},
            {field: 'separation', title: '检测间隔'},
            {field: 'fail', title: '失败条件'},
            {field: 'recovery', title: '恢复条件'},
	...
	table.on('toolbar(list-table)', function(obj) {
		...
		if (obj.event === 'add') {
			layer.open({
				type: 2,
				title: '添加',
				content: '/menu/redirect_heart/form',
				maxmin: true,
				area: ['550px', '550px'],
				btn: ['确定', '取消'],
				yes: function(index, layero) {
					...
					iframeWindow.layui.form.on('submit(form-add-submit)', function(data) {
						var field = data.field;
						if (field.enable != null && field.enable == 'on'){
							field['enable'] = true;
						}else {
							field['enable'] = false;
						}
			...
			layer.open({
				...
				yes: function(index, layero) {
					...
					iframeWindow.layui.form.on('submit(form-edit-submit)', function(formData) {
						...
						if (resultData.enable != null && resultData.enable == 'on'){
							resultData['enable'] = true;
						}else {
							resultData['enable'] = false;
						}
