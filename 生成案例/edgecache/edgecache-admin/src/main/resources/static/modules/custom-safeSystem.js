/**
 @Name：SafeSystem JS
 */
layui.define(['table', 'form','element','laydate'], function(exports) {
	var $ = layui.$,
		table = layui.table,
		form = layui.form,
		element = layui.element,
		laydate = layui.laydate,
		moduleUrl = "/safe_system/";
	table.render({
		elem: '#list-table',
		url: moduleUrl + 'page',
		method: 'post',
		...
		cols: [[
            {field: 'networkSegment', title: '允许网段'},
	...
	form.on('submit(list-table-search2)', function(data) {
		var field = data.field;
		$.each(field,function (k, v) {
			if (v === ""){
				delete field[k];
