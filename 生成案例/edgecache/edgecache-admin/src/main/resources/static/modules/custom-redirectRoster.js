/**
 @Name：RedirectRoster JS
 */
layui.define(['table', 'form'], function(exports) {
	var $ = layui.$,
		table = layui.table,
		form = layui.form,
		moduleUrl = "/redirect_roster/";
	table.render({
		elem: '#list-table',
		url: moduleUrl + 'page',
		method: 'post',
		...
		cols: [[
            ...
            {field: 'id', title: 'id'},
			{field: 'type', title: '名单类型',templet:function (d) {
					return d.type == 1 ? "白名单":"黑名单"
			}},
            {field: 'addr', title: '地址类型',templet:function (d) {
				return d.addr == 1 ? "源地址":"目标地址"
            }},
            {field: 'url', title: '值'},
	...
	table.on('toolbar(list-table)', function(obj) {
		...
		if (obj.event === 'add') {
			layer.open({
				type: 2,
				title: '添加',
				content: '/menu/redirect_roster/form',
				maxmin: true,
				area: ['550px', '550px'],
				btn: ['确定', '取消'],
