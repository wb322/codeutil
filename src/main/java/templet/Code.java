	...
	public static void create(Map<String,String> pathMap,Map<String,String> publicMap)
	{
		...
		for(Templet t:list  )
		{
			...
			if( FileUtil.isTemplatFile( t.getAllPath()) ){
				...
				//如果文件名包含表替换符号则循环输出 <wb 路径中包含表替换符号/>
				if(t.getFileName().indexOf("[table]")>=0  || t.getFileName().indexOf("[Table]")>=0  || 
					...
					|| t.getPath ().contains ("[Table]")
					|| t.getPath ().contains ("[table]")
					|| t.getPath ().contains ("[table2]")
					|| t.getPath ().contains ("[Table2]")){
					//<wb 将文件路径记录/>
					String pathBefore = t.getPath ();
					for(Table table:tableList)
					{
						//<wb 将文件路径还原含表替换符号/>
						t.setPath (pathBefore);
						...
						pathAfter = pathAfter.replace("[table2]", table.getName2() );
						t.setPath (pathAfter);
